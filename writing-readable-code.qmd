...
![The machine does the first thing and the last thing. Everything in between is yours.](figs/review-process-machine-then-person.png){fig-alt="Three stacked bands. The top grey band is step 1, fix the style, done by a machine in seconds, with pills for air format and styler. An arrow leads down to a purple band holding steps 2 to 6, labelled only a person can do these: read it top to bottom and mark it up, check the names, group the similar parts together, review the comments, and remove the wilted lettuce. A second arrow leads down to another grey band, step 7, run a linter, a machine again once you have finished moving things, with pills for jarl check and lintr." width=100%}
...
We did this one at length in [Bad, better, and good variable names](#bad-better-and-good-variable-names), because it is the step with the best return for the effort.
...
### 4. Group the similar parts together {#sec-de-chunking}
...
### Re-expression: can you say it more simply?
