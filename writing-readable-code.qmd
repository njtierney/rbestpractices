...
[Project organisation -@sec-project-organisation] was about where things live. A project that opens, paths that work on someone else's machine, a README that says what the thing is.

[Code style and readability -@sec-code-style] was about how the code looks. Pick a naming convention. Let `air` or {styler} handle the layout. Let a linter like {jarl} or {flir} tell you about the rest. 
...
```r
df1 <- read.csv("survey.csv")
df2 <- df1[df1$year == 2024, ]
df3 <- df2[df2$site == "north", ]
```
...
```r
survey_raw <- read.csv("survey.csv")
survey_2024 <- survey_raw[survey_raw$year == 2024, ]
survey_north <- survey_2024[survey_2024$site == "north", ]
```
