...
There is more to your code than having it run. I mean, it **should run**, and that is the first step. But this is a book on **best practices**, so we are aiming for your code to not just be "running", but "easy to read, and easy to understand".
...
One of these is harder to read than the other, and I think this is because one of them has a few spelling mistakes, and doesn't clearly express what it says.

Now, writing is hard! I'm not saying it is easy. It takes time, it takes iteration. You have to revisit things. My point is: text is easier to understand when the spelling is correct, the grammar is good.

Punctuation does the same job, and sometimes it does more than that. Sometimes it decides what the words mean.
...
You can still work out what it says. But you spend energy doing it. Our precious mental energy. If you're reading something:

<!-- Note: example of the following -->

and these little bumps interrupt your thinking:
...
M<-mean(d$weight)
S<-sd(d$weight)
t<-tapply(d$weight,d$Diet,mean)
t2<-tapply(d$weight,d$Diet,length)
r<-data.frame(D=names(t),m=as.numeric(t),n=as.numeric(t2))
r<-r[order(-r$m),]
r$f<-r$m>M
r
```
...
```{r}
head(ChickWeight)
day_21 <- ChickWeight$Time == 21
chicks_day_21 <- ChickWeight[day_21, ]

weight_mean <- mean(chicks_day_21$weight)
weight_sd <- sd(chicks_day_21$weight)

mean_by_diet <- tapply(chicks_day_21$weight, chicks_day_21$Diet, mean)
chicks_by_diet <- tapply(chicks_day_21$weight, chicks_day_21$Diet, length)

diet_summary <- data.frame(
  diet = names(mean_by_diet),
  weight_mean = as.numeric(mean_by_diet),
  n_chicks = as.numeric(chicks_by_diet)
)

diet_summary <- diet_summary[order(-diet_summary$weight_mean), ]
diet_summary$above_overall_mean <- diet_summary$weight_mean > weight_mean
...
Here's the good news. Nearly all of that surface level noise is systematic, which means we can get rid of it systematically. Quite a lot of it we can hand to a computer entirely.
...
That's the whole argument, and the sentence performs it on you as you read it.

There is a comprehensive guide in the [Tidyverse style guide](https://style.tidyverse.org/), and a shorter one in the [style chapter of Advanced R, 1st edition](http://adv-r.had.co.nz/Style.html).
...
```r
# correctness
if (all.equal(x, y)) "same" else "different"
if (isTRUE(all.equal(x, y))) "same" else "different"
```

Both of those are laid out perfectly well. Only one of them is right, and we come back to why later in this chapter. That is a *linter's* job.
...
The third one is the interesting case, because it is the one that looks fine. Every space is in the right place, the indentation is correct, and `air` would leave it exactly as it is.
...
::: {.callout-caution title="How deep can this go?" collapse="true"}

Here is the same summary again, with the read and a rename folded in as well.
...
Five levels deep. The first thing that happens is on line five, and the last thing that happens is on line one.

Nothing here is badly laid out. That is the point.
...
The second set is not wrong, exactly. It just means that every time you want one of those objects you have to recall how you happened to feel on the day you made it.
...
[Air](https://posit-dev.github.io/air/) is an R formatter from Posit. You point it at your code, and it lays it out properly.

It is basically magic, and it changes how you work almost immediately.

Once you have it wired into your editor, you stop thinking about layout. You type whatever comes out of your head, hit save, and it comes out tidy. That's it. That's the whole workflow.

### Checking air is installed

You installed air in [Code formatters](setup.qmd#sec-code-formatters). We just need to check it is there, and find out where it lives.

::: {.callout-note title="Your Turn: check air is installed"}

Do this one now, before we go any further, because everything in the rest of this section needs it.
...
Sometimes the install just doesn't go. You're on a locked down work machine, you can't get at a terminal, or the installer refuses and won't tell you why.

You're not stuck.

[`{styler}`](https://styler.r-lib.org/) is an R package that formats your code, and it installs the way every other R package you have ever used installs:
...
It follows the same tidyverse style guide air does, so the output is very close. Not identical, but close enough that nothing in this chapter changes. Everywhere air comes up from here on, there is a styler tab showing you the same thing.

Now the honest bit.

Air is faster, and it hooks into format on save, which is the whole point of the next section. With styler you have to remember to run it.

That is a real difference and I'm not going to pretend otherwise.

But code you formatted by hand is still formatted, and it beats a mess. If air won't install, take styler, keep moving, and sort air out another day.

## Format on save, which is the good bit

This is the part that changes how you work, so I want to be clear about what it gives you.

Once this is set up, you write code however it falls out of your head. Wonky spacing, arguments strewn across the line, indentation all over the place. You do not fix any of it.

Then you hit `Cmd / Ctrl + S`, and it is all just correct.

Not "you run a command and it tidies up". You hit save, the thing you already do a hundred times a day, and the file is formatted. There is no new habit to build, because you are attaching it to a habit you already have.

I cannot really oversell this. It is the single highest value setting in this whole chapter, and it takes two minutes.
...
Set **Code formatter** to **External**.
...
It sets up the editor configuration and adds an `air.toml` to your project.
...
I want you to do it repeatedly, because the point is not that it worked once. The point is to stop believing you have to tidy your own code.
...
There is also an RStudio addin, under **Addins → Style active file**, which formats whatever file you are looking at. If you are going down this road, bind it to a keyboard shortcut, because you will use it constantly.
...
2. Format it. With air, run `air format 06-bad-style/messy-analysis.R` from your terminal. With styler, run `styler::style_file("06-bad-style/messy-analysis.R")`, or open the file and use **Addins → Style active file**.
...
Step 4 is the important one. It's the clearest demonstration I know that layout and naming are two different jobs.
...
**There is very little to configure, and that's on purpose.** Air's settings live in an `air.toml` file, and there are about ten of them. The ones you might actually change are `line-width` (default 80) and `indent-width` (default 2). There is no way to spend an afternoon tuning it, which is the point.
...
The name is the good part. Lint is the fluff that clothes shed in the wash, and a clothes dryer has a lint trap to catch it. Johnson's idea was that his program would work the same way, catching the waste fibres while leaving the fabric intact.
...
Every answer is `NA`, including for the values that are obviously not missing. Your `missing_mass` vector contains no information at all, and nothing warned you.
...
This one is not a bug. It is a mental cycles problem, which is the theme of this whole chapter.
...
That second one is a string, and `if()` can't do anything sensible with it. So the code works perfectly while your values match, and falls over the first time they don't, which is precisely the case you wrote the check for. `isTRUE(all.equal(x, y))` fixes it.
...
The two in the callout above **error**, which sounds better and is arguably worse. They work fine while the data behaves, so they pass every test you run on tidy data, and then fail on a Friday afternoon on somebody else's file.

That is what a linter is for. No amount of careful reading catches all of these reliably, and a tool catches them every time in about a tenth of a second.
...
A linter you run on every save changes your habits. A linter that takes eighteen seconds is one you run occasionally, and then stop running.
...
Call them with `flir::` in front, as above. If you run `library(flir)` instead, its `fix()` will mask the `fix()` that comes with R, and you'll get a warning that has nothing to do with your code.

That second one is why I'm pointing you at flir rather than at `{lintr}`. If you can't install jarl, flir is the only one of these that will still fix things for you.
...
- Style splits into layout, which a tool can fix, and naming, which is yours.
