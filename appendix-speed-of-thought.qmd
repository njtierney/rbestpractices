...
Positron asks a bit more of you, because there's no list to click. You write the binding yourself, in `keybindings.json`. There's a worked example of that, for reprex, in [Writing a reprex / getting unstuck](writing-a-reprex-getting-unstuck.qmd).

::: {.callout-important title="Addins in Positron"}
...
2. Install `{fnmate}` or `{datapasta}`, and bind it to a key. Tools → Modify Keyboard Shortcuts in RStudio, or a line in `keybindings.json` in Positron.
